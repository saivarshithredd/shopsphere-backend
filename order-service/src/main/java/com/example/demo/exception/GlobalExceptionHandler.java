...
public class GlobalExceptionHandler {
    ...
    // Feign errors — order-service calls catalog-service
    ...
    public ResponseEntity<ErrorResponse> handleFeign(FeignException ex) {
        ...
        String message = switch (status) {
            ...
            case 401 -> "Unauthorized call to catalog service";
            case 403 -> "Access denied by catalog service";
            default  -> "Error calling catalog service: " + ex.getMessage();
        };
        return ResponseEntity
            .status(status)
